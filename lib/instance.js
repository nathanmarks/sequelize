    ...
    if (!options.raw && this._customSetters[key]) {
      ...
      if (this.$options && this.$options.include && this.$options.includeNames.indexOf(key) !== -1) {
        ...
        if (!options.raw) {
          ...
          if (
            !this.isNewRecord
            && this.Model._hasReadOnlyAttributes
            && this.Model._isReadOnlyAttribute(key)
            && !(options.manualUpdate && this.Model._isTimestampAttribute(key))
          ) {
            return;
          }
...
Instance.prototype.save = function(options) {
  ...
  return Promise.bind(this).then(function() {
    ...
    .then(function() {
      ...
      if (updatedAtAttr && !options.silent && !(options.manualUpdate && values[updatedAtAttr])) {
